...
functions such that the length of transient and that of 
the cycle the transient leads to are both exponential in $L$
(see Theorem~\ref{thm:path_length_lower_bounds}).
This idea enables us to establish \cpsp-hardness result for
the reachability problem for DAG-SyDSs.
...
functions are NCFs and whose directed graphs may contain cycles
is \cpsp-complete.
To our knowledge, reachability problem for DAG-SyDSs 
